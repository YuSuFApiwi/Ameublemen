...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($nom) && !empty($telephone) && !empty($addresse) && !empty($email)) {
        ...
        <div style="padding: 1.5rem;padding-bottom: 0">
            <h3 class="gry-color text-center" style="background: #f2f2f2;padding:10px">Désignation : RIDEAU REF <span class="strong black-color">'.$_POST["ref"].'</span></h3>
        </div>';
        ...
        $element3 = "";
        if (!empty($_POST["tringle"]) || !empty($_POST["parte_embrasse"]) || !empty($_POST["embrasse"])) {
        ...
        if (!empty($_POST["tringle"])) {
            $body .='<tr class="strong">
            <td class="gry-color">Tringle</td>
            <td class="strong black-color">'.$_POST["tringle"].'</td>
            <td class="strong black-color">'.$_POST["qte_tringle"].'</td></tr>';
        }
        if (!empty($_POST["embrasse"])) {
            $body .='<tr class="strong">
            <td class="gry-color">Embrasse</td>
            <td class="strong black-color">'.$_POST["embrasse"].'</td>
            <td class="strong black-color">'.$_POST["qte_embrasse"].'</td></tr>';
        }
        if (!empty($_POST["parte_embrasse"])) {
            $body .='<tr class="strong">
            <td class="gry-color">Parte Embrasse</td>
            <td class="strong black-color">'.$_POST["parte_embrasse"].'</td>
            <td class="strong black-color">'.$_POST["qte_parte_embrasse"].'</td></tr>';
        }
...
<body>
	<div style="margin-left:auto;margin-right:auto;">
		...
		$subject_client = "Validation Commande - iyaameublement.com";
        $headers_client = "From: Commande <contact@apiwi-multimedia.com>\r\n";
        $headers_client .= "MIME-Version: 1.0\r\n";
        $headers_client .= "Content-type: text/html; charset-utf-8";
        
        $subject = "Demande de Prix - (Rideau)" . date("d-m-20y");
